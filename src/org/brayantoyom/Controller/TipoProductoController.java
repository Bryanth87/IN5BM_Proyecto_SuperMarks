...
public class TipoProductoController implements Initializable {
    ...
    public void Guardar() {
        ...
        try {
            PreparedStatement procedimiento = Conexion.getInstance().getConexion().prepareCall("{sp_agregarTipoProducto(?, ?)}");
        ...
        switch (tipoDeOperaciones) {
            ...
            default:
                if (tblTipoProducto.getSelectionModel().getSelectedItem() != null) {
                    int respuesta = JOptionPane.showConfirmDialog(null, "¿Estás seguro de eliminarlo?", "Eliminar producto",
                            JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                    if (respuesta == JOptionPane.YES_NO_OPTION) {
                        try {
                            PreparedStatement procedimiento = Conexion.getInstance().getConexion().prepareCall("{sp_eliminarTipoProducto(?)}");
                    ...
                    JOptionPane.showMessageDialog(null, "Primero selecciona un producto para eliminar");
                }
        }
    }
        ...
        switch (tipoDeOperaciones) {
            case NINGUNO:
                if (tblTipoProducto.getSelectionModel().getSelectedItem() != null) {
                    ...
                    JOptionPane.showMessageDialog(null, "Primero selecciona un producto para editar");
                }
                break;
            case ACTUALIZAR:
                Actualizar();
    ...
    public void Actualizar() {
        ...
        try {
            PreparedStatement procedimiento = Conexion.getInstance().getConexion().prepareCall("{sp_actualizarTipoProducto(?, ?)}");
